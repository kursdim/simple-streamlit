...
k = st.number_input("Введите k(Коэффицент наклона начальной линейной функции. Чем больше k, тем больше прямая 'прижимается' к оси Y", value=0)
x_0 = st.number_input("Введите x_0(Начальное значение суммы продаж, с которого начинается премия)", value=0)
x_1 = st.number_input("Введите x_1(Сумма продаж, при которых премирование начинает быть нелинейным)", value=0)
y_0 = st.number_input("Введите y_0(Начальная сумма премии)", value=0)
y_1 = st.number_input("Введите y_1(Значение, больше которого премия не может быть)", value=0)
x_val = st.number_input("Введите x(Суммы продаж, для которой нужно посчитать значение премии)", value=0)
