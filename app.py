...
k = st.number_input("**k** (Коэффицент наклона начальной линейной функции. Чем больше k, тем больше прямая 'прижимается' к оси Y", value=0)
x_0 = st.number_input("**x_0** (Начальное значение суммы продаж, с которого начинается премия)", value=0)
x_1 = st.number_input("**x_1** (Сумма продаж, при которых премирование начинает быть нелинейным)", value=0)
y_0 = st.number_input("**y_0** (Начальная сумма премии)", value=0)
y_1 = st.number_input("**y_1** (Значение, больше которого премия не может быть)", value=0)
x_val = st.number_input("**x** (Сумма продаж, для которой нужно посчитать значение премии)", value=0)
...
@st.fragment
def show_value():
    if st.button("Рассчитать значение в точке **x**"):
        if k*x_1 + (y_0 - k*x_0) >= y_1:
            st.write('Неккоректные значения параметров: значение линейной функции в точке x1 больше y1. Измените параметры')
        else:
            a = (y_1 - y_0 + k * (x_0 - x_1)) ** 2 / k
